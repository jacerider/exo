 ...
 * @FieldWidget(
 *   id = "exo_autocomplete",
 *   label = @Translation("eXo Autocomplete (Tags style)"),
 *   field_types = {
 *     "entity_reference",
 ...
 * )
 */
class ExoAutocompleteWidget extends WidgetBase implements ContainerFactoryPluginInterface {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $element += [
      '#multiple' => $multiple,
      '#exo_autocomplete_path' => preg_replace('/^[^\\/]+/', '', Url::fromRoute('exo.autocomplete', $route_parameters, ['absolute' => TRUE])->toString()),
      ...
      foreach ($values['target_id'] as $key => $value) {
        if ($value['target_id']) {
          ...
          if ($entity instanceof RevisionableInterface) {
            ...
            $values['target_id'][$key]['target_revision_id'] = $entity->getRevisionId();
          }
        }
        ...
        if (is_array($value['target_id'])) {
          unset($values['target_id'][$key]['target_id']);
          $values['target_id'][$key] += $value['target_id'];
        }
      }
    }
