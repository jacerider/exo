 ...
 *
 * @package Drupal\exo\Plugin
 */
class ExoExtraFieldDisplayManager extends DefaultPluginManager implements ExoExtraFieldDisplayManagerInterface {
  ...
  /**
   * Constructor for ExoExtraFieldDisplayManager objects.
   *
  ...
  public function __construct(\Traversable $namespaces, CacheBackendInterface $cache_backend, ModuleHandlerInterface $module_handler, EntityTypeManagerInterface $entity_type_manager) {
    parent::__construct('Plugin/ExtraField/Display', $namespaces, $module_handler, 'Drupal\exo\Plugin\ExoExtraFieldDisplayInterface', 'Drupal\exo\Annotation\ExoExtraFieldDisplay');
    $this->alterInfo('exo_extra_field_display_info');
    $this->setCacheBackend($cache_backend, 'exo_extra_field_display_plugins');
   ...
   * {@inheritdoc}
   */
  public function pluginView($pluginId, ContentEntityInterface $entity, $viewMode = 'default') {
    $build = [];
    $display = \Drupal::entityTypeManager()->getStorage('entity_view_display')->load($entity->getEntityTypeId() . '.' . $entity->bundle() . '.' . $viewMode);
    ...
    if ($this->hasDefinition($pluginId)) {
      ...
      if ($this->matchEntityBundleKey($definition['bundles'], $entityBundleKey)) {
        ...
        if ($display->getComponent($this->fieldName($pluginId))) {
          ...
          /** @var ExoExtraFieldDisplayInterface $plugin */
