...
use Drupal\exo_filter\ExoFilterHelper;
use Drupal\exo_filter\Plugin\ExoFilterBase;
use Drupal\exo_filter\Plugin\ExoFilterInterface;

/**
 * Base class for Better exposed pager widget plugins.
 */
abstract class SortWidgetBase extends ExoFilterBase implements ExoFilterInterface {
...
Title Desc|Z -> A</pre> Leave the replacement text blank to remove an option altogether. The order the options appear will be changed to match the order of options in this field.'),
      '#states' => [
        'visible' => [
          ':input[name="exposed_form_options[sort][configuration][advanced][combine]"]' => ['checked' => TRUE],
        ],
      ],
    ];
    ...
    $form['advanced']['reset_label'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="exposed_form_options[sort][configuration][advanced][reset]"]' => ['checked' => TRUE],
        ],
        'required' => [
          ':input[name="exposed_form_options[sort][configuration][advanced][reset]"]' => ['checked' => TRUE],
        ],
    ...
    $form['advanced']['collapsible_label'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="exposed_form_options[sort][configuration][advanced][collapsible]"]' => ['checked' => TRUE],
        ],
        'required' => [
          ':input[name="exposed_form_options[sort][configuration][advanced][collapsible]"]' => ['checked' => TRUE],
        ],
  ...
  public function exposedFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    if ($this->configuration['advanced']['combine'] && !empty($form['sort_order'])) {
      ...
      if (!empty($this->configuration['advanced']['combine_rewrite'])) {
        $options = ExoFilterHelper::rewriteOptions($options, $this->configuration['advanced']['combine_rewrite'], TRUE);
