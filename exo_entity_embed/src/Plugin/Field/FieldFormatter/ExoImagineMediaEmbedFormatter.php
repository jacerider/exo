...
use Drupal\exo_imagine\Plugin\Field\FieldFormatter\ExoImagineMediaFormatter;

/**
 * Plugin implementation of the 'exo image' formatter.
 *
 * @FieldFormatter(
 *   id = "exo_image_media_embed",
 *   label = @Translation("eXo Imagine Embed"),
 ...
 * )
 */
class ExoImagineMediaEmbedFormatter extends ExoImagineMediaFormatter {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    if (\Drupal::routeMatch()->getRouteName() === 'embed.preview') {
      // When in preview, we want to show immediately.
      $this->exoImagineSettings->setSetting('visible', 0);
      $this->exoImagineSettings->setSetting('animate', 0);
      $this->exoImagineSettings->setSetting('blur', 0);
    }
    $elements = parent::viewElements($items, $langcode);
    foreach ($elements as $key => &$element) {
      $element['#attributes']['class'][] = 'exo-embed-alignment-' . $this->getSetting('alignment');
      $element['#attributes']['class'][] = 'exo-embed-size-' . $this->getSetting('size');
      if ($uri = $this->getSetting('uri')) {
        $url = Url::fromUri($uri);
        $element['#tag'] = 'a';
        $element['#attributes']['href'] = $url->toString();
