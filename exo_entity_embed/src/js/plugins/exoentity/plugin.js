...
(function ($, Drupal, CKEDITOR) {
  ...
  CKEDITOR.plugins.add('exoentity', {
    ...
    beforeInit: function (editor) {
      ...
      editor.addCommand('editexoentity', {
      ...
      if (editor.ui.addButton) {
        for (var key in editor.config.ExoEntity_buttons) {
          var button = editor.config.ExoEntity_buttons[key];
          editor.ui.addButton(button.id, {
            ...
            click: function(editor) {
              editor.execCommand('editexoentity', this.data);
            },
            // icon: button.image,
            icon: '_ exo-icon exo-icon-font icon-' + button.icon + '  _'
          });
        }
      }
      ...
      if (editor.contextMenu) {
        editor.addMenuGroup('exoentity');
        editor.addMenuItem('exoentity', {
          label: Drupal.t('Edit Entity'),
          icon: this.path + 'entity.png',
          command: 'editexoentity',
          group: 'exoentity'
        });

        editor.contextMenu.addListener(function(element) {
          if (isEditableEntityWidget(editor, element)) {
            return { exoentity: CKEDITOR.TRISTATE_OFF };
          }
        });
      }
      ...
      editor.on('doubleclick', function (evt) {
        ...
        if (isEditableEntityWidget(editor, element)) {
          editor.execCommand('editexoentity');
        }
      });
    }
  });
  ...
  /**
   * Get the surrounding exoentity widget element.
   *
  ...
  /**
   * Checks if the given element is an editable exoentity widget.
   *
   * @param {CKEDITOR.editor} editor
  ...
  function isEditableEntityWidget (editor, element) {
    ...
    if (!widget || widget.name !== 'exoentity') {
      return false;
    }
    ...
    return editor.config.ExoEntity_buttons.hasOwnProperty(button);
  }
