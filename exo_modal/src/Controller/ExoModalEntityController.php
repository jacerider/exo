...
class ExoModalEntityController extends ControllerBase {
   ...
   * Update entity form.
   */
  public function deleteForm(Request $request, EntityInterface $entity, $revision_id = NULL) {
    $build = [
      '#title' => $this->t('Edit %label', ['%label' => $entity->label()]),
  ...
  public function createForm(Request $request, $entity_type, $bundle, $display_id, $data = []) {
    ...
    if (!$this->entityTypeManager->getAccessControlHandler($entity_type->id())->createAccess($bundle, $this->currentUser())) {
