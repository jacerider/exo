class ExoVideos extends ExoDataManager<ExoVideoBase> {
  protected label:string = 'ExoVideo';
  protected settingsGroup:string = 'exoVideo';
  protected instanceSettingsGroup: string = 'videos';
  protected autoplaySupported:boolean = null;
  protected autoplayPromise:Promise<boolean>;
  protected readonly events = {
    ready: new ExoEvent<ExoVideoBase>(),
    timeupdate: new ExoEvent<ExoVideoBase>()
  };
  ...
  protected createInstance(id:string, data:any) {
    if (typeof Drupal.ExoVideoProviders[data.provider] !== 'undefined') {
      const $element = $('#' + id);
      if ($element.length) {
        return new Drupal.ExoVideoProviders[data.provider](id, $element);
      }
    }
    return false;
  }
  ...
  public videoResize() {
    this.getInstances().each(instance => {
      instance.videoResize();
    });
  }

  public onReady(video: ExoVideoBase) {
    this.event('ready').trigger(video);
  }

  public onTimeUpdate(video:ExoVideoBase) {
    this.event('timeupdate').trigger(video);
  }
  ...
  public detectAutoplay(acceptableDelay: number):Promise<boolean> {
    ...
    this.autoplayPromise = new Promise((resolve, reject) => {
      ...
      const autoplay_test_content = document.createElement('video');
      ...
      var base64_test_video = document.getElementById("base64_test_video") as HTMLVideoElement;
      //test for autoplay, 100 ms buffer
      setTimeout(() => {
        this.autoplaySupported = !base64_test_video.paused;
        resolve(this.autoplaySupported);
